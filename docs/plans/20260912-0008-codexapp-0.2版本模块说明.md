...
| 自动化存储/历史 | `automationStore.ts`、`automationHistory.ts` | 单写者租约、hot state、按任务/日期分段冷归档、永久 request key 事实与可恢复索引 |
...
0.2.19 的定期 inspect 在串行控制区外等待，最多四个未结束的底层检查，同一 run 只允许一个。检查持有克隆快照，回写核对归属、revision、状态与通知版本；完成/取消/审批之后的旧快照不得覆盖。手动控制及其他账号可用槽位不等待该查询。准备链现在由 `AutomationPreparation` 管理 90 秒期限、取消与真实清理；新建准备进程确认退出后释放，已有共享进程的已发操作保留所有权至返回。只读原生请求超时仍保留实际 pending，最多四个/进程并合并未完成的同参数准备读取。详见本轮最终收口验收的 SCH-03 边界。
...
`AutomationHistory` 的 history-v2 保存两份永久请求事实（records/keys）、pending 归档事务以及按任务/UTC 日期的 timeline 和按 ID 的派生索引。只有完整归档成功，引擎才从 hot state 移出终态；热冷交界按原 runId/时间去重。分页最多 100 条、8 路正文读取、32 个日索引缓存；防重查询直接按键散列读取，不扫描全部历史。旧源文件保留，首次目录变更恢复后复制到新格式。

`automationHistoryMaintenance.ts` 提供显式离线修复/旧版导出，必须取得同一个调度锁，不启动账号或原生服务。历史查询可能触发恢复，因此引擎也校验锁归属。回退到旧应用之前，用当前包执行 `automation-history --home <目标 CODEX_HOME> --export-legacy`；目标调度器必须停止。不会删除永久防重事实，也不重写账号和热运行格式。进程中断五阶段已有真实测试；全部事实丢失或断电不是已验证的恢复承诺。
