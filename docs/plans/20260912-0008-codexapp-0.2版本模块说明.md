...
旧 `accountActivationSession.ts` 是先前通过 app-server 激活的实现，只剩测试引用。当前调度不调用它；列为退役候选，不作为现行模块扩展点。
...
提交源码 → build → pack → 新目录安装 → 组件校验 → CLI/PTY/真实包缓存验收 → 精确路径交接。未来授权激活时需 drain/idle 后才停服务、保留认证回退信息并切换；本次停在 prepare。SCH-01 提醒当前发布门禁尚未覆盖全部激活动作。
