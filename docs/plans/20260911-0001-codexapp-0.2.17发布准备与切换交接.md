...
用户反馈这两个关键词查不到技能。实际 59001 页面在原始清单下均返回 0；它的 `/codex-api/skills-hub` 只返回 39 个技能，名称、简介、作者和路径中均没有这两个词。强制刷新后仍为 39 项。容器 `/codex-home/skills` 仅有 `.system` 和 `AGENTS.md`，没有本机自定义技能，也没有失效的同名链接。

只读对照生产 5900：清单为 51 项，包含 `homepage-maintenance`、`x-bot-maintenance` 和 `telegram-bridge-send`。仅在测试浏览器中临时替换 59001 技能列表响应为这份真实元数据，随后操作同一个筛选框：

| 数据来源 | homepage | bot | 筛选新增请求 |
| --- | --- | --- | --- |
| 59001 原始 39 项 | 0 | 0 | 0 |
| 浏览器内临时加载主机 51 项元数据 | homepage-maintenance | telegram-bridge-send、x-bot-maintenance | 0 |

Telegram 技能名称不含 bot，通过简介正确命中；清空筛选恢复完整数量。结论是独立候选环境未安装这些技能，未发现名称/简介筛选失效。上一轮简介用例仅覆盖当时已有的技能，不能证明候选技能清单与主机一致。

本次只读检查，浏览器响应替换随上下文关闭而消失：没有复制技能、修改账号/认证、重建候选、启动 4173 或更新 prepared 包。测试没有后端写请求，页面错误为 0。发布目录及代码哈希不变。

验证 URL `http://127.0.0.1:59001/#/skills?tab=skills`，1440×1000 中文浅色；截取实际已安装技能控件。脚本/结果位于 `/home/Code/codexapp/output/0217-prepare/filter-homepage-bot.cjs`、`filter-homepage-bot.json`。原始截图为 `/home/Code/codexapp/output/playwright/0217-filter-candidate-homepage.png`、`0217-filter-candidate-bot.png`、`0217-filter-host-metadata-in-browser-only-homepage.png`、`0217-filter-host-metadata-in-browser-only-bot.png`。

![候选原始数据：homepage 无匹配](assets/0.2.17-prepare/0217-filter-candidate-homepage.png)
![浏览器内加载主机元数据：homepage 命中](assets/0.2.17-prepare/0217-filter-host-metadata-in-browser-only-homepage.png)
![浏览器内加载主机元数据：bot 命中名称与简介](assets/0.2.17-prepare/0217-filter-host-metadata-in-browser-only-bot.png)
