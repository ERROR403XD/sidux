...
用户随后授权处理 59001 并准备切换。18:31（Asia/Shanghai）完成以下变更与核验：

- 先检查开发服务：活跃回合、队列、审批、API 和后台终端均为 0；自动激活关闭，没有忙碌执行归属。
- 新建 `/etc/systemd/system/codexapp-dev-59001.service`，通过 `systemd-analyze verify`。保留工作目录 `/home/Code/codexapp`、原启动参数、独立 home 与原 API 组件路径；私有环境文件为 `/home/docker/codexapp-dev-0218/service.env`（0600）。不继承父会话标识、生产端口变量或临时 Codex PATH。
- 核对原后台终端 processId、itemId、命令、cwd 与监听进程后，通过 `thread/backgroundTerminals/terminate` 停止该终端，确认旧监听结束，再启动独立服务。
- 新 PID `3181332`，控制组 `/system.slice/codexapp-dev-59001.service`；生产仍为 PID `3840130`，两个端口各只有一个监听进程。开发服务没有生产服务依赖，重启生产不再连带停止它。
- 59001 capabilities 返回 HTTP 200、appVersion=0.2.18，空闲检查通过。原开发 home 原地保留，认证文件哈希一致；`accounts.json` 启动前后字节不同，未宣称所有运行元数据不变，也未回写旧元数据。
- 完整枚举生产 14 个已加载会话，后台终端会话数为 **0**；原终端记录消失。生产自动激活仍关闭，无未结束激活。
- 运行验证：`node -e` 加载 `node-pty` 的 `spawn` 为 function，实际 PTY 输出 `DEV_SERVICE_PTY_OK`、退出 0；复用已验证的现有构建，没有修改包或模块。
- 运行开销检查：单一 59001 监听，无重复实例，`NRestarts=0`；启动后控制组内存约 123 MiB、48 个 task。未做真实模型请求或长期性能测试。

服务为手动启动的 static unit，不新增开机自启。开发服务操作使用 `systemctl start|stop|restart codexapp-dev-59001.service`；配置变动先 `systemctl daemon-reload`。若要撤销托管，先停止独立开发服务，再处理 unit 和环境文件；不删除独立 home，不恢复成生产会话中的长期后台终端。

此时生产精确路径门禁仍报告 `liveExecutions=2`、`activeTurns=2`，队列、审批和 API 均为 0。59001 阻塞已解除，尚需结束活跃回合，从独立终端复核后执行已准备版本的切换。未执行生产重启、activate 或账号切换。
