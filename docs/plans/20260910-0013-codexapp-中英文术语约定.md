...
- 账号卡片按用户后续要求只显示时长：`7天`、`30天` / `7 days`、`30 days`，省略额度 / quota；标签列恢复固定 5.7em，保持同一卡片两行进度条起点及长度一致。无时长数据时显示 `—`，无障碍标签保留剩余额度含义。
...
此处复核替代上方旧截图中的额度标签布局。最新镜像为 `sha256:8ecde7f60f4e3554b07798fb51ff0757bc2d816160bee969437977a85b3e0df4`，已部署 59001，27 个构建文件与本地一致，5900 进程未改变。

- 翻译及额度展示相关 13 项测试通过，构建通过。
- 请求拦截样本验证 7 天与 30 天窗口：1440×1000、375×812、768×1024，各覆盖中英文及深浅主题，共 12 组通过。账号卡片两行进度条宽度与起点一致；左侧只显示时长，单行且不覆盖进度条。
- 性能代码审查：仅固定 CSS 列宽及 6 个静态词条，时长转换为常数时间计算；没有新增请求、计时器、监听器或文件访问。未重复进行整站性能采样。
- 独立观察：原有右侧英文重置时间在部分额度行有约 3px 的 scrollWidth 溢出；此次没有扩大调整右侧时间列，也不宣称整个额度行完全无溢出。

[12 组 UI 结果](assets/0.2.16-short-quota/ui.json) · [13 项测试](assets/0.2.16-short-quota/unit.log) · [产物校验](assets/0.2.16-short-quota/artifacts.json)

测试 URL：`http://127.0.0.1:59001/#/settings`。截图源绝对路径为 `/home/Code/codexapp/output/playwright/0216-short-quota-375-light-en.png` 与 `/home/Code/codexapp/output/playwright/0216-short-quota-375-dark-zh-CN.png`。

![手机英文浅色账号卡片，375×812](assets/0.2.16-short-quota/0216-short-quota-375-light-en.png)

![手机中文深色账号卡片，375×812](assets/0.2.16-short-quota/0216-short-quota-375-dark-zh-CN.png)
