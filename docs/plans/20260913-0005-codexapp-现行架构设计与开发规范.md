...
自动化完成先归档，归档成功后才能裁剪热状态。历史分页缓存可淘汰，永久请求事实不能随缓存删除。只有旧索引而正文丢失时拒绝重放，不能当作“未找到请求”。
...
| 项目组织 | `codexapp-projects.json` | VirtualProjectStore；名称、稳定 ID、成员 cwd，仅 UI 归组 |
...
| 草稿、偏好、显示投影 | 当前来源 localStorage | 按浏览器/端口隔离；显示投影不是服务端投递账本 |
