...
const release = '/home/docker/codexapp-releases/codexapp-0.2.19-df6a5cb97c32-20260913-025347';
...
async function main() {
  ...
  fs.writeFileSync('output/0219-final/sealed-npm-ci.log', ci.stdout + ci.stderr);
...
main().then(() => { report.passed = true; }).catch(e => { report.error = e.message; process.exitCode = 1; }).finally(async () => {
  ...
  fs.writeFileSync('output/0219-final/sealed-prepared-verification.json', JSON.stringify(report, null, 2) + '\n');
