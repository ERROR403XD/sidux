...
| `AppSwitch.vue` | `modelValue`、`disabled`；标签插槽；`update:modelValue`、`change` 事件 | 原生 button 的键盘操作和 switch 语义；不请求接口或保存业务数据 |
...
- ResizeObserver 回调不直接写入会影响被观察元素或父布局的样式；将测量与更新安排到下一帧，合并目标并跳过等价尺寸，卸载取消 RAF。标题和侧栏已按此规范修复 WebKit 的反馈循环。
- 有边框的 AppSelect 触发器与浮层一起检查浅/深主题，使用共用 `--ui-field`、`--ui-text` 和 `--ui-border`。懒加载组件的 scoped 样式可能晚于全局 CSS 注入，决定性主题规则放在 `src/style.css` 并确保优先级足够；不能只检查展开后的选项面板。
...
父菜单判断外部点击时使用 `isOverlayEventInside(event, container)`；不要仅用 `container.contains(event.target)`。AppPopover 在显示时登记 panel 与触发器的归属，嵌套浮层仍属于父菜单；卸载时清理监听与登记。不通过全局 stopPropagation 解决下拉框误关闭。Pad 触摸和展开输入区定位的修复证据见 [0018](20260906-0018-codexapp-0.1.90封测收尾与Pad修复交接.md)。

## 启动失败恢复界面补充

启动恢复提示必须独立于 Vue 主模块和外链 CSS，放在入口 HTML；否则主模块失败时恢复入口也会失效。正常挂载后沿用共用 UI 色彩，冷启动时使用独立的系统/已保存主题样式。禁止自动循环刷新；已挂载页面允许稍后处理，不自动清空或发送草稿。恢复与主题验证见 [0021 最终封版](20260906-0021-codexapp-0.1.90白屏修复与最终封版.md)。


## 0.2.17 设置与层级补充

- 普通设置使用 `settings-form-grid`；组件内部使用 `settings-form-subgrid` 继承列宽，不添加二次缩进。内容宽度达到 600px 时两栏，其余单栏。同级字段共用 14px 上下留白、分隔线和控件高度，错误信息单独占行。
- 短选择器与标签同行；`settings-field-short` 使用较短宽度，`settings-field-roomy` 给模型、时区、标题分配更多空间。菜单跟随触发框宽度；不要为了统一样式把短字段拆成上下两行。
- 当前字号：页面标题 22px（手机 18px）、分区标题 16px、子标题 14px、字段/动作 13px、辅助数据 12px。普通字段高 36px、动作高 32px；聊天正文、账号额度摘要和输入防缩放等专用区域保留各自规则。
- 布尔偏好使用 AppSwitch。免打扰使用“标签 → switch → 起始 至 结束”同行，时间控件和连接词均为 12px；关闭时保持可见并 disabled，不收缩布局。
- 改动布局时核验中英文、明暗主题、375/768/1440px，以及字段/下拉菜单实际边界；不要仅检查页面成功加载。

## 操作通知（0.2.18）

用户操作失败和重要操作成功统一使用 `notifyOperation(message, kind, scope?)`，由根级 `OperationToastHost.vue` 渲染。纯 UI 状态不作为成功依据；成功通知在操作接口确认后发出。会话专属通知携带会话 ID，离开会话时清理。

成功 3 秒、失败 6 秒；鼠标悬停或键盘焦点进入时暂停，× 关闭，同类替换，最多 3 条。沿用 `--ui-*` 主题、圆角、焦点和浮层层级，不抢焦点、不挤压输入框、不新增请求。

系统自动加载失败、持续功能受限、上游回合错误仍留在原有区域。不得按报错文本关键词或 HTTP 状态码决定是否 Toast；调用方根据用户操作/后台加载的来源区分。关闭通知不解除业务保护，也不等于忽略额度错误。

Toast 正文直接显示具体结果，不添加“操作提示：”“提示：”等引导前缀或额外标题；通知容器顶边对齐页面工具栏底边，向下堆叠，避免遮挡 Git／新建会话操作。
