...
status: 已实施并部署 59001；核心验收通过，待用户封测
---

# CodexApp 0.2.0 反代 API 出口实施方案

> 实施结果：G0–G3 核心开发与候选验证已完成；交付、298 项测试、真实无 HTTP 回退的 Codex WS 编程、UI/性能及待封测项见 [0025](20260906-0025-codexapp-0.2.0API出口实施与验收.md)。下文原计划不代表当前部署状态。

> 2026-09-06 执行补充：用户已明确“开始执行”。G0 发现旧 `/responses/compact` 在测试账号的直连上游同样返回 404；官方 0.153.4 的 Responses V2 压缩可用。采用小范围端点适配：追加原生 `compaction_trigger`，只接受真实加密 compaction 输出，保留调用方 user 消息，并验证压缩后续接。没有自制聊天总结或修改组件源码。组件启动加 `-local-model`，锁定内嵌目录。证据见 [G0 验证](evidence/0.2.0-api-proxy-g0.json)。下文“仅文档”描述保留为原规划轮的授权边界。
>
> 令牌发布采用有界进程代际：新访问令牌启动并验证新进程后才接新请求；旧进程仅完成已接收请求，至多两代。它比依赖文件 watcher 的确认事件更容易验证，不向任一组件进程提供 refresh token。账号切换仍先 drain，不借代际切换绕开活动检查。独立固定账号选择纳入本期。
