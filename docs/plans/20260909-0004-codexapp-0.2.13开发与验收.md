...
created: 2026-09-09
updated: 2026-09-09
status: prepare已完成；59001已部署；未切换5900
---
...
59001保留运行；本轮4173验证服务已通过process-activity/terminate终止，端口复查已无监听，5173未操作。5900继续运行0.2.12，未执行activate/publish/merge。回滚候选仍需先核对空闲，再按部署脚本使用旧验证镜像，禁止两个容器同时写候选卷。认证测试容器已停止，假数据卷保留供核查。

X-bot为单独项目修复，详见同日x-bot抽奖JSON文档；不包含在CodexApp包内。

## Prepare与切换交接

2026-09-09 22:40完成prepare，发布目录为：

`/home/docker/codexapp-releases/codexapp-0.2.13-e306b944a937-20260909-224004`

包对应提交`e306b944a937`；后续仅更新本文档，不改变已验收包。`latest-prepared`已指向该目录。20个CLI/前端产物逐文件一致，CLI与59001一致；公共CJS入口检查通过，真实node-pty命令输出`PREPARED_PTY_OK`并正常退出。结果见`output/0213/prepare-verification.json`，构建安装记录见`prepare.log`。

- CLI SHA256：`36fa528f5d4a7fa4c1b2013da42b01c1baf734f937772bf785263db410e9ecc0`
- tarball SHA256：`a883663b05e0e13fdbee6fef5336f6c9dd115ca296c43f80a7d6fb5280ac6ad8`
- 候选缓存实测：HTML与条件请求均200、no-store；哈希资源200且immutable；缺失旧资源404，见`prepare-cache.json`。

精确路径check退出3：`activeTurns=1|queued=0|pendingApprovals=0|apiConnections=0|apiActiveRequests=0|backgroundThreads=skipped-busy|loadedThreads=0|pages=2`。当前对话仍在运行，后台线程检查因忙碌被跳过；这不是切换门禁通过。4173已停止，5900/59001监听正常。没有绕过门禁或执行切换。

结束本次对话、关闭CodexApp页面后，在独立终端重新检查；仅在决定切换且检查通过时执行activate：

```bash
cd /home/Code/codexapp
release=/home/docker/codexapp-releases/codexapp-0.2.13-e306b944a937-20260909-224004
...
如再次报告忙碌，先等待对应任务完成，不绕过门禁。切换后仍需检查systemd、5900、HTTP、活动账号及真实请求；本次prepare未执行这些切换后验收，也未消费真实重置机会。
