...
status: 追加复审发现待处理问题；候选仍在59001；未激活生产
tags: [开发, CodexApp, 封测, 验收, 运维]
---

# CodexApp 0.2 封测验收与运行维护

**追加复审说明：** 重新运行全部 815 项测试仍通过，但新增交叉/故障探针确认四项遗漏，其中自动化保存竞争和组织移除后的自动化失效影响执行可靠性。下文是原封测范围和结果，不能据此继续认定所有问题均已闭环。完整结论见 [[20260913-0010-codexapp-0.2封测全量复审与需求结账]]；本次只审查，未修复或切换固定包。

0.2.19 的本轮开发和追加修复已完成；追加代码完成后另跑了一轮系统性全量测试。当前封测候选运行在 59001，生产 5900 保持原 0.2.18。此结论限于下面列明的验证环境，不等同真实云账号、多日连续运行或所有操作系统均已验收。
...
| 应用源码基线 | `df6a5cb97c32`；后续文档和测试诊断提交不改变该应用包 |
| prepare 目录 | `/home/docker/codexapp-releases/codexapp-0.2.19-df6a5cb97c32-20260913-025347` |
| 候选入口 | `http://192.168.50.46:59001/`；本机验证 `http://127.0.0.1:59001/` |
| 候选服务 | `codexapp-dev-59001.service`；独立 `CODEX_HOME=/home/docker/codexapp-dev-0218/codex-home` |
| 生产 | `codexapp.service`，5900，原 `codexapp-0.2.18-6ad0e99242c8-20260912-221253`，`/root/.codex` |
| 验证服务 | `http://127.0.0.1:4173/`，当前仓库、独立测试 home；保留运行 |
| 运行环境 | Linux、Node 24.19.0、系统 Chromium；打包认证矩阵使用原生 Codex 0.153.4 |
| 内置 API 组件 | CLIProxyAPI 7.2.152，prepare 校验通过 |

tarball SHA-256：`6cae1206b249ee201bc8afa638028088e35f275e7cfbf299352d9dbd353efa3a`。固定安装目录的 package-lock SHA-256：`8e6560139ec4babe3e19fdd974452eeb4be4ade6b45a9b6dbaa7e0ccede1b43a`。这是安装产物校验值，不包含认证信息。

59001 更新前后独立检查：activation、liveExecutions、activeTurns、queued、pendingApprovals、apiConnections、apiActiveRequests、backgroundThreads 均为 0；后台检查受支持，loadedThreads=0。不是 skipped-busy 或 unsupported。生产 PID 与认证文件对本轮私有基线仅作一致性比较，报告只保留布尔结果；未重启、切换生产或修改生产账号配置。
...
```bash
node -e "process.argv=['node','codexapp','--help']; require('/home/docker/codexapp-releases/codexapp-0.2.19-df6a5cb97c32-20260913-025347/dist-cli/index.js')"
```
...
| 现象 | 先检查 | 避免的动作 |
| --- | --- | --- |
| 引导内容等待/未知 | 投递 ID、queue/receipt、原生回显与显示缓存 | 新建 ID 连续重发或直接清空台账 |
...
```bash
node /home/docker/codexapp-releases/codexapp-0.2.19-df6a5cb97c32-20260913-025347/dist-cli/index.js automation-history --home /明确的目标CODEX_HOME --export-legacy
```
...
Obsidian 已确认目标为 `开发/codexapp/history`。旧版本材料和旧证据集中归档，新的两份 0.3 UI 设计稿及 `assets/0.3-ui-design/` 留在原位。当前六份文档以本页及导航互相链接；本次迁移 96 份根目录历史材料及配套素材，共 831 个文件；保留两份设计稿及 14 个设计素材文件。核对 490 个原有可解析链接，仅调整设计稿内 5 个指向历史材料的相对链接。归档清单保存迁移前后哈希和这些调整，确保归档可以核对。
