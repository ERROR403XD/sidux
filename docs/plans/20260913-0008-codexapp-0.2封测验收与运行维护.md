...
status: 四项复审缺陷已修复并回归；修复候选在59001；未激活生产
tags: [开发, CodexApp, 封测, 验收, 运维]
---

# CodexApp 0.2 封测验收与运行维护

**修复收口：** REV-01–04 已按局部修复方案完成；最新全量为 **121 个文件、823 项通过，0 失败、0 跳过**。后台保存/新入队、移除组织后的手动/自然到点/固定账号/重试、缓存写满刷新、挂起摘要后连续忽略均有新增回归。最新故障注入、打包/复装、性能和运行证据见 [[20260913-0011-codexapp-0.2封测复审问题修复与回归]]；第 3–5 节保留上一阶段封测的完整记录，不将旧测试次数冒充本次重跑。
...
| 应用源码基线 | 固定包来源 `0612357254c4`；最后产品修复提交 `04ca8cc`，后续 `74e9321` 只扩展组合测试 |
| prepare 目录 | `/home/docker/codexapp-releases/codexapp-0.2.19-0612357254c4-20260913-101637` |
...
tarball SHA-256：`59bc618fa3cc090837689367a5fbe5dbf4b644e4299ebad394c61be7ff7e7520`。固定安装目录的 package-lock SHA-256：`8e6560139ec4babe3e19fdd974452eeb4be4ade6b45a9b6dbaa7e0ccede1b43a`。这是安装产物校验值，不包含认证信息。

59001 更新前后独立检查：activation、liveExecutions、activeTurns、queued、pendingApprovals、apiConnections、apiActiveRequests、backgroundThreads 均为 0；后台检查受支持，loadedThreads=0。不是 skipped-busy 或 unsupported。此次更新 59001 前后生产 PID 均为 4010970；未重启、切换生产或修改生产账号配置。首次候选 drain 遇到账号操作忙返回 409 时已停止替换并解除冻结；再次按同一门禁与完整空闲检查通过后才更新，没有强制放行。原封测另有生产认证私有基线比对；本次不把它算作重新实测。
...
```bash
node /home/docker/codexapp-releases/codexapp-0.2.19-0612357254c4-20260913-101637/dist-cli/index.js automation-history --home /明确的目标CODEX_HOME --export-legacy
```
...
最终公开版本 0.2.19 已合入 PR #6 并发布；开发/公共源码均 826/826。当前 59001 与 prepared 路径均为 `/home/docker/codexapp-releases/codexapp-0.2.19-d20d9fe539a3-20260913-105947`。5900 仍为 0.2.18。临时测试服务已清理；当前 check 被活跃回合阻挡，后台终端本次跳过，须独立终端重新检查。 详见 [[20260913-0012-codexapp-封版漏项补查与0.2.19交接]]。
