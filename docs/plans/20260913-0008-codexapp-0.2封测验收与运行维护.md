...
updated: 2026-09-13
version: 0.2.19
baseline: d20d9fe539a3
status: 0.2.19已封版；2026-09-13 11:32只读查验生产5900与候选59001均运行固定发布包
tags: [开发, CodexApp, 封测, 验收, 运维]
---

# CodexApp 0.2 封测验收与运行维护

**当前维护基线：** 0.2.19 已公开发布并 prepare；最终开发/公开源码各 **121 个文件、826 项通过，0 失败、0 跳过**。包含 REV-01–04、空项目折叠和新增三项存储失败/重启回归。最新交接见 [[20260913-0012-codexapp-封版漏项补查与0.2.19交接]]；0011 的 823 项及下方第 3–5 节的更早验收均为历史阶段，不算本次重新测试。

2026-09-13 11:32（上海时间）文档同步时只读查验：生产 5900 与候选 59001 均已运行下表中的固定 0.2.19。封版交付时生产仍为 0.2.18，后续已完成切换；此次文档工作没有执行 activate、重启或更改配置，也未重跑生产真实请求验收。

## 1. 固定产物与实例

| 项目 | 当前已查验记录 |
| --- | --- |
| 开发分支 | `feature/0.2.19-stabilization`；仅内部开发和文档历史 |
| 公开应用基线 | `d20d9fe539a3`；GitHub main/tag 合并提交 `c2a4661bfac1`，两棵文件树相同 |
| 固定发布目录 | `/home/docker/codexapp-releases/codexapp-0.2.19-d20d9fe539a3-20260913-105947` |
| 候选入口 | `http://192.168.50.46:59001/`；本机验证 `http://127.0.0.1:59001/` |
| 候选服务 | `codexapp-dev-59001.service`，PID 2544578；独立 `CODEX_HOME=/home/docker/codexapp-dev-0218/codex-home` |
| 生产 | `codexapp.service`，5900，PID 2623228；11:28:27 启动，使用上述固定包；原独立 `/root/.codex` |
| 查验方式 | 实际 MainPID 的启动命令与两实例 `/codex-api/meta/capabilities` 均确认 0.2.19；不据此声称全局空闲 |
| 临时验证服务 | 封版交接已停止 4173、59015、4195–4197；没有重启这些服务 |
| 验证环境 | Linux、Node 24.19.0、系统 Chromium；隔离打包认证矩阵使用 Codex 0.153.4 |
| API 组件 | CLIProxyAPI 7.2.152，prepare 校验通过 |

公开 tarball SHA-256：`01893f892715d26a971a5d8c62ba0e136d31e9cdc41513d36d15e6a74d82cedc`。固定包 package-lock SHA-256：`8e6560139ec4babe3e19fdd974452eeb4be4ade6b45a9b6dbaa7e0ccede1b43a`。来源见 0012 精简证据，不包含认证信息。

最后一次由本会话执行的 59001 更新按空闲检查→冻结→复查→替换→解除冻结完成，前后生产 PID 4010970 未变。之后用户要求查询并停止另一会话的后台终端，复查两实例后台终端均为 0；这是当时的独立终端清单，不是今后的全局空闲凭证。生产后续切换后的新 PID 2623228 是本次文档同步时的只读发现。
