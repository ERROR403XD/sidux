...
if (process.env.CODEX_HOME !== '/codex-home') throw new Error('Run this fixture inside the isolated Sidux container');
...
fs.writeFileSync(path.join(root,'AGENTS.md'),'这是 Sidux 0.1.90 的虚构验收目录。所有输入与输出都必须在当前目录中；不要读取外部路径，不使用网络服务。\n');
...
fs.writeFileSync(path.join(root,'README.md'),'Sidux 0.1.90 内部验收：全部数据为虚构。验收时间范围固定为 2026-08-24 至 2026-08-30。输出日历/fixture-0190.md。\n');
