...
const release = '/home/docker/codexapp-releases/codexapp-0.2.19-3a9b9e1391b7-20260912-225317';
...
async function main() {
  ...
  fs.writeFileSync('output/0219/npm-ci.log', ci.stdout + ci.stderr);
...
main().then(() => { report.passed = true; }).catch(e => { report.error = e.message; process.exitCode = 1; }).finally(async () => {
  ...
  fs.writeFileSync('output/0219/prepared-verification.json', JSON.stringify(report, null, 2) + '\n');
