// Review-only defect probe: real packaged HTTP/engine, synthetic native IPC.
...
async function main() {
  ...
  const report = { fixturePort: port, cliSha256: createHash('sha256').update(await fs.readFile(cli)).digest('hex'), requests: [], faultConfirmed: false }
  ...
  try {
    ...
    report.faultConfirmed = after.status === 'failed' && report.definitionRetainedAfterRemoval
    assert.ok(report.faultConfirmed)
  } finally {
    child.kill('SIGTERM')
    ...
    await fs.writeFile('output/0219-review/virtual-automation.json', JSON.stringify(report, null, 2) + '\n')
  }
}
