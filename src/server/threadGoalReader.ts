import type { ThreadGoal } from '../api/threadCommands.js'
...
export class ThreadGoalReader {
  ...
  observe(notification: { method: string; params: unknown }) {
    ...
    if (notification.method === 'thread/goal/updated' && params.goal) this.put(params.threadId, params.goal)
  ...
  snapshot(ids: string[]): Promise<Record<string, ThreadGoal | null>> {
    ...
    const execute = async () => {
      ...
      for (let offset = 0; offset < unique.length; offset += 4) {
        const reads = await Promise.allSettled(unique.slice(offset, offset + 4).map(async id => {
          ...
          if (cached && this.now() - cached.at < 30000) { result[id] = cached.goal; return }
          ...
          if (this.cache.get(id) === cached) this.put(id, response.goal)
