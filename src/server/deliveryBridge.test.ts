...
async function fixture() {
  ...
  const rpc = vi.fn(async (method: string): Promise<unknown> => {
    if (method === 'thread/read') return { thread: { id: 'fixture', status: { type: busy ? 'active' : 'idle' } } }
  ...
  return { processor, rpc, message, busy: (value: boolean) => { busy = value } }
}
...
describe('durable bridge delivery', () => {
  ...
  it('deduplicates a lost HTTP response and exposes no private params', async () => {
    ...
    const input = { protocol: 2, threadId: 'fixture', message, params: { threadId: 'fixture', input: [{ type: 'text', text: message.text }], model: message.model } }
  ...
  it('queues an ordinary send when a stale page thinks an active thread is idle', async () => {
    ...
    busy(true)
    const response = await processor.submit({ protocol: 2, threadId: 'fixture', message, params: { threadId: 'fixture', input: [] } })
  ...
  it('keeps edited rows in place, preserves settings, and rejects stale versions', async () => {
    ...
    busy(true)
    await processor.mutate({ protocol: 2, type: 'add', threadId: 'fixture', message })
    const editing = await processor.mutate({ protocol: 2, type: 'edit', threadId: 'fixture', messageId: message.id, revision: 1, editToken: 'tab-one' })
    expect(editing.state.fixture[0].delivery?.status).toBe('editing')
    await expect(processor.mutate({ protocol: 2, type: 'remove', threadId: 'fixture', messageId: message.id, revision: 1 })).rejects.toThrow()
    const updated = await processor.mutate({ protocol: 2, type: 'update', threadId: 'fixture', messageId: message.id, revision: 2, editToken: 'tab-one', message: { ...message, text: 'edited', model: 'other' } })
  ...
  it('blocks submissions while provider mutation owns the gate', async () => {
    ...
    await expect(processor.mutate({ protocol: 2, type: 'add', threadId: 'fixture', message })).rejects.toThrow('账号操作')
    release()
    expect(await processor.readState()).toEqual({})
  })
})
