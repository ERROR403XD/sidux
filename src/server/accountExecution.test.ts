...
it('normal work preempts only same-account activation, including idle-to-busy transitions', () => {
  const registry = new AccountExecutionRegistry()
  const normal = registry.register({storageId:'a',kind:'primary',ownerId:'chat',busy:false,disconnect:vi.fn()})
  const close = vi.fn()
  const activation = registry.register({storageId:'a',kind:'activation',ownerId:'activation',disconnect:close})
  const other = registry.register({storageId:'b',kind:'activation',ownerId:'other',disconnect:vi.fn()})
  normal.setBusy(true)
  expect(activation.signal.aborted).toBe(true)
  expect(close).toHaveBeenCalledOnce()
  expect(other.signal.aborted).toBe(false)
  expect(normal.signal.aborted).toBe(false)
  const second = registry.register({storageId:'b',kind:'api',ownerId:'api',disconnect:vi.fn()})
  expect(other.signal.aborted).toBe(true)
  expect(second.signal.aborted).toBe(false)
})

it('background allocations never evict normal account components', async () => {
  const dispose = vi.fn()
  const pool = new AccountResourcePool({capacity:1,idle:()=>true,dispose})
  const first = await pool.getOrCreate('a',()=>({}))
  expect(await pool.getOrCreate('b',()=>({}),false)).toBeNull()
