  ...
  <section class="account-panel custom-connections">
    ...
    <AppDialog :open="dialog" :title="t(draft.storageId ? '账号设置' : '添加账号')" :busy="busy" panel-class="custom-connection-dialog" @close="dialog = false">
      <div class="custom-connection-form">
        ...
        <div v-if="bridgeToggleAvailable" class="custom-connection-wide custom-connection-bridge">
          <AppSwitch v-model="draft.protocolBridge" :disabled="busy">{{ t('协议转换') }}</AppSwitch>
          <p class="account-panel-notice">{{ t('连接缺少的端点经协议桥转换后可用（黄色标记）。') }}</p>
        </div>
...
// must be savable without re-probing the provider.
...
watch(proofSignature, () => { testToken.value = ''; testedEndpoints.value = []; confirmRemove.value = false })
