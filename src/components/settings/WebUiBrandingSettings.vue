<template>
  <div class="webui-branding-settings">
    ...
    <div class="sidebar-settings-row sidebar-settings-row--select"><label for="webui-title">{{ t('WebUI标题') }}</label><input id="webui-title" v-model="title" class="app-input" placeholder="Codex Web" maxlength="120" :disabled="busy" @change="save({ title })" /></div>
    <div class="sidebar-settings-row sidebar-settings-row--select"><span>{{ t('标题显示') }}</span><AppSelect :model-value="branding.titleMode" :options="titleOptions" :aria-label="t('标题显示')" :disabled="busy" @update:model-value="save({ titleMode: $event })" /></div>
